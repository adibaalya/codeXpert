...
<body class="code-test-body">
    ...
    <script>
        let editor;
        let hasRunCode = false; // Track if code has been run
        const language = "{{ $question->language ?? 'Python' }}";
        const languageMap = { 'Java': 'java', 'Python': 'python', 'JavaScript': 'javascript', 'C++': 'cpp', 'C#': 'csharp', 'PHP': 'php', 'Ruby': 'ruby', 'Go': 'go', 'C': 'c' };
        ...
        // FIXED: Simply toggle DISPLAY block/none. Flex logic handles the height.
        ...
        function runCode() {
            ...
            fetch('{{ route("learner.coding.run") }}', {
                ...
                body: JSON.stringify({ code: code, language: language, question_id: '{{ $question->question_ID }}' })
            })
            ...
            .then(text => {
                ...
                if (data.success) {
                    // Mark that code has been run successfully
                    hasRunCode = true;
                    
                    // Enable submit button and update validation message
                    const submitBtn = document.getElementById('submitBtn');
                    ...
                    // Show overall status (Accepted or Failed) based on validation
                    ...
                    if (data.testResults && Array.isArray(data.testResults)) {
                        data.testResults.forEach((testResult, index) => {
                            ...
                            // Expected Output - ALWAYS show if it exists
        ...
        function rateQuestion(rating) {
            const goodBtn = document.getElementById('goodBtn'); const badBtn = document.getElementById('badBtn');
            ...
            fetch('{{ route("learner.coding.rate") }}', {
                ...
                body: JSON.stringify({ question_id: '{{ $question->question_ID }}', rating: rating })
        ...
        function getNextQuestion() {
            ...
            params.append('exclude', '{{ $question->question_ID }}');
            
            if ('{{ $question->language }}') params.append('language', '{{ $question->language }}');
            if ('{{ $question->level }}') params.append('level', '{{ $question->level }}');
            if ('{{ $question->chapter }}') params.append('topic', '{{ $question->chapter }}');
